...
// Check slider status.
add_filter( 'bizroot_filter_slider_status', 'bizroot_check_slider_status' );

// Add slider to the theme.
add_action( 'bizroot_action_before_content', 'bizroot_add_featured_slider', 5 );

// Slider details.
add_filter( 'bizroot_filter_slider_details', 'bizroot_get_slider_details' );


if ( ! function_exists( 'bizroot_get_slider_details' ) ) :
	/**
	 * Slider details.
	 *
	 * @since 1.0.0
	 ...
	 */
	function bizroot_get_slider_details( $input ) {

		$featured_slider_type           = bizroot_get_option( 'featured_slider_type' );
		$featured_slider_number         = bizroot_get_option( 'featured_slider_number' );
		$featured_slider_read_more_text = bizroot_get_option( 'featured_slider_read_more_text' );
		...
		switch ( $featured_slider_type ) {
			...
			case 'featured-page':
				...
				for ( $i = 1; $i <= $featured_slider_number ; $i++ ) {
					$id = bizroot_get_option( 'featured_slider_page_' . $i );
					if ( ! empty( $id ) ) {
						$ids[] = absint( $id );
					}
				}
				...
				if ( ! empty( $all_posts ) ) {
					...
					foreach ( $all_posts as $key => $post ) {
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							$image_array = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'bizroot-slider' );
							$slides[ $cnt ]['images']  = $image_array;
							$slides[ $cnt ]['title']   = esc_html( $post->post_title );
							$slides[ $cnt ]['url']     = esc_url( get_permalink( $post->ID ) );
							$slides[ $cnt ]['excerpt'] = bizroot_the_excerpt( apply_filters( 'bizroot_filter_slider_caption_length', 30 ), $post );
...
if ( ! function_exists( 'bizroot_add_featured_slider' ) ) :
	/**
	 * Add featured slider.
	 *
	 * @since 1.0.0
	 */
	function bizroot_add_featured_slider() {

		$flag_apply_slider = apply_filters( 'bizroot_filter_slider_status', false );
		if ( true !== $flag_apply_slider ) {
			return false;
		}

		$slider_details = array();
		$slider_details = apply_filters( 'bizroot_filter_slider_details', $slider_details );
		...
		// Render slider now.
		bizroot_render_featured_slider( $slider_details );

	}
endif;


if ( ! function_exists( 'bizroot_render_featured_slider' ) ) :
	/**
	 ...
	 */
	function bizroot_render_featured_slider( $slider_details = array() ) {

		if ( empty( $slider_details ) ) {
			return;
		}
		$featured_slider_transition_effect   = bizroot_get_option( 'featured_slider_transition_effect' );
		$featured_slider_enable_caption      = bizroot_get_option( 'featured_slider_enable_caption' );
		$featured_slider_enable_arrow        = bizroot_get_option( 'featured_slider_enable_arrow' );
		$featured_slider_enable_pager        = bizroot_get_option( 'featured_slider_enable_pager' );
		$featured_slider_enable_autoplay     = bizroot_get_option( 'featured_slider_enable_autoplay' );
		$featured_slider_enable_overlay      = bizroot_get_option( 'featured_slider_enable_overlay' );
		$featured_slider_transition_duration = bizroot_get_option( 'featured_slider_transition_duration' );
		$featured_slider_transition_delay    = bizroot_get_option( 'featured_slider_transition_delay' );
...
if( ! function_exists( 'bizroot_check_slider_status' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function bizroot_check_slider_status( $input ) {
